...
import { ArrowRight, Bot, Package, RotateCcw, Truck, Zap, ShieldCheck, BarChart3, MessageSquare, CheckCircle2, Clock, Star, ArrowUpRight } from "lucide-react";
  ...
  return (
    <div className="min-h-dvh bg-white">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-5xl px-6 pb-20 pt-20 sm:pt-28 text-center">
          ...
          <p className="animate-slide-up stagger-1 mx-auto mb-10 max-w-xl text-base text-zinc-500 leading-relaxed sm:text-lg">
          ...
          {/* Stats */}
          <div className="animate-slide-up stagger-3 mx-auto mt-16 grid max-w-md grid-cols-3 gap-8 rounded-2xl border border-zinc-100 bg-zinc-50/50 px-6 py-5 backdrop-blur-sm">
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <div className="rounded-2xl border border-red-100 bg-white p-6 shadow-sm transition-all hover:shadow-md">
              ...
              <div className="space-y-2">
                {[
                  { side: "user", text: "I want to return my shoes" },
                  { side: "bot", text: "Here's our return policy: [link]" },
                  { side: "user", text: "Where do I go?" },
                  { side: "bot", text: "Visit our returns portal and enter your order #" },
                  { side: "user", text: "What's my order number?" },
                  { side: "fade", text: "... 10 minutes later, still stuck ..." },
                ].map((l, i) => (
                  <div key={i} className={`rounded-lg px-3 py-2 text-xs leading-relaxed ${l.side === "user" ? "ml-6 bg-zinc-100 text-zinc-700" : l.side === "fade" ? "bg-red-50 text-red-400 text-center italic" : "mr-6 bg-zinc-50 text-zinc-600"}`}>
                    {l.side !== "fade" && <span className="font-medium text-zinc-400">{l.side === "user" ? "Customer" : "Bot"}: </span>}
                    {l.text}
                  </div>
                ))}
              </div>
            ...
            <div className="rounded-2xl border border-emerald-100 bg-white p-6 shadow-sm transition-all hover:shadow-md">
              ...
              <div className="space-y-2">
                ...
                <div className="mr-6 rounded-lg border border-violet-100 bg-violet-50 px-3 py-2 text-xs text-violet-700">
                  <span className="mr-1">📦</span> Bot renders order cards with product images
                </div>
                <div className="ml-6 rounded-lg bg-zinc-100 px-3 py-2 text-xs text-zinc-700"><span className="font-medium text-zinc-400">Customer: </span>Taps the sneakers</div>
                <div className="mr-6 rounded-lg border border-violet-100 bg-violet-50 px-3 py-2 text-xs text-violet-700">
                  <span className="mr-1">🔄</span> Bot renders exchange form with size picker
                </div>
