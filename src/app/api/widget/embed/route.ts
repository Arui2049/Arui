...
export async function GET(req: Request) {
  ...
  const triggerSelectorParam = searchParams.get("trigger");
  const autoOpenParam = searchParams.get("autoOpen");
  const offsetRightParam = searchParams.get("offsetRight");
  const offsetBottomParam = searchParams.get("offsetBottom");
  ...
  // Put token in hash fragment to reduce leakage via Referer / server logs.
  ...
  const launcher = launcherParam == null ? true : launcherParam !== "0";
  const triggerSelector = triggerSelectorParam ? escapeJS(triggerSelectorParam) : "";
  const autoOpen = autoOpenParam === "1";
  const offsetRight = Math.max(0, Math.min(200, Number(offsetRightParam || "20") || 20));
  const offsetBottom = Math.max(0, Math.min(200, Number(offsetBottomParam || "20") || 20));
  ...
  var LAUNCHER=${launcher ? "true" : "false"};
  var TRIGGER_SELECTOR="${triggerSelector}";
  var AUTO_OPEN=${autoOpen ? "true" : "false"};
  var OFFSET_RIGHT=${offsetRight};
  var OFFSET_BOTTOM=${offsetBottom};
  ...
  frame.setAttribute("style",isMobile?"position:fixed;bottom:0;right:0;left:0;top:0;z-index:2147483646;display:none;":"position:fixed;bottom:"+(OFFSET_BOTTOM+68)+"px;right:"+OFFSET_RIGHT+"px;z-index:2147483646;width:380px;height:560px;max-height:calc(100vh - "+(OFFSET_BOTTOM+88)+"px);border-radius:16px;overflow:hidden;box-shadow:0 8px 32px rgba(0,0,0,0.16);display:none;");
  ...
  function setLauncherVisible(visible){
    try{ if(btn){ btn.style.display=visible?"block":"none"; } }catch(e){}
  }
  function setOpen(next){
    open=!!next;
    frame.style.display=open?"block":"none";
    if(isMobile && btn){btn.style.display=open?"none":"block";}
    if(btn){
      ...
      if(toggle){ toggle.innerHTML=svg; }
    }
    if(open){track("widget_open",{mobile:isMobile});}
  }
  function openWidget(){ setOpen(true); }
  function closeWidget(){ setOpen(false); }
  function toggleWidget(){ setOpen(!open); }

  // Expose stable API for integrating with existing support entry
  try{
    window.ReturnbotWidget=window.ReturnbotWidget||{};
    window.ReturnbotWidget.open=openWidget;
    window.ReturnbotWidget.close=closeWidget;
    window.ReturnbotWidget.toggle=toggleWidget;
    window.ReturnbotWidget.isOpen=function(){return open;};
    window.ReturnbotWidget.showLauncher=function(){setLauncherVisible(true);};
    window.ReturnbotWidget.hideLauncher=function(){setLauncherVisible(false);};
  }catch(e){}

  if(LAUNCHER){
    btn=document.createElement("div");
    btn.id="returnbot-widget";
    btn.setAttribute("style","position:fixed;bottom:"+OFFSET_BOTTOM+"px;right:"+OFFSET_RIGHT+"px;z-index:2147483647;");
    ...
    document.getElementById("rb-toggle").addEventListener("click",function(){ toggleWidget(); });
  }

  function bindTriggers(){
    if(!TRIGGER_SELECTOR) return;
    try{
      var els=document.querySelectorAll(TRIGGER_SELECTOR);
      for(var i=0;i<els.length;i++){
        els[i].addEventListener("click",function(ev){ try{ev.preventDefault();}catch(e){} openWidget(); });
      }
    }catch(e){}
  }
  ...
  window.addEventListener("message",function(e){
    if(e.data==="rb-close"){closeWidget();}
  });
})();`;
  ...
  return new Response(js, {
    headers: {
      ...
      // Token is short-lived; avoid caching a stale token.
